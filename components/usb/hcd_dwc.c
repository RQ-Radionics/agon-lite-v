...
struct pipe_obj {
    ...
    // SPLIT transaction info (for FS/LS devices behind a HS hub)
    uint8_t hub_addr;                       // USB address of the upstream HS hub (0 = no SPLIT needed)
    uint8_t hub_port;                       // Port number on that hub (1-based)
    // Port related
...
esp_err_t hcd_pipe_alloc(hcd_port_handle_t port_hdl, const hcd_pipe_config_t *pipe_config, hcd_pipe_handle_t *pipe_hdl)
{
    ...
    // hub_addr/hub_port are set separately via hcd_pipe_set_hub_info()
...
static void IRAM_ATTR _buffer_exec(pipe_t *pipe)
{
    ...
    // Program HCSPLT register for FS/LS devices behind a HS hub (SPLIT transactions)
    // The DWC scatter-gather DMA engine handles SSPLIT/CSPLIT automatically once spltena=1.
    if (pipe->hub_addr != 0) {
        usb_dwc_hcsplt_reg_t splt = {
            .hubaddr  = pipe->hub_addr,
            .prtaddr  = pipe->hub_port,
            .xactpos  = 3,      // 0b11 = All (entire FS payload fits in one HS transaction)
            .compsplt = 0,      // Hardware toggles this; start with SSPLIT
            .spltena  = 1,
        };
        pipe->chan_obj->regs->hcsplt_reg = splt;
    } else {
        pipe->chan_obj->regs->hcsplt_reg.val = 0; // No SPLIT
    }
