...
static esp_err_t endpoint_alloc(device_t *dev_obj, const usb_ep_desc_t *ep_desc, usbh_ep_config_t *ep_config, endpoint_t **ep_obj_ret)
{
    ...
    // For FS/LS devices behind a HS hub, set hub address and port for SPLIT transactions
    if (dev_obj->constant.parent_dev_hdl != NULL) {
        uint8_t hub_addr = 0;
        usbh_dev_get_addr(dev_obj->constant.parent_dev_hdl, &hub_addr);
        hcd_pipe_set_hub_info(pipe_hdl, hub_addr, dev_obj->constant.parent_port_num);
    }
...
static esp_err_t device_alloc(usbh_dev_params_t *params, device_t **dev_obj_ret)
{
    ...
    if (params->parent_dev_hdl != NULL) {
        uint8_t hub_addr = 0;
        usbh_dev_get_addr(params->parent_dev_hdl, &hub_addr);
        hcd_pipe_set_hub_info(default_pipe_hdl, hub_addr, params->parent_port_num);
    }
